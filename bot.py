...
def start(message):
    ...
    bot.send_message(message.chat.id, 'Привет, я бот, можешь смотреть или создать заявку', reply_markup=keyboard)
...
def orders(message):
    ...
    bot.send_message(message.chat.id, 'Заявки:', reply_markup=keyboard)

@bot.callback_query_handler(func=lambda call: call.data == 'quit')
def start(call):
    keyboard = types.ReplyKeyboardMarkup(True, True)
    keyboard.row('Просмотреть заявки', 'Создать заявку')
    bot.send_message(chat_id=call.message.chat.id, text='Привет, я бот, можешь смотреть или создать заявку', reply_markup=keyboard)
...
def callback_next(call):
    ...
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='Заявки, страница ', reply_markup=keyboard)
